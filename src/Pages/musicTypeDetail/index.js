...
const MusicTypeDetail = () => {
  ...
  const handleEditChange = (e) => {
    ...
    setEditedItem((prevState) => ({
      ...prevState,
      [field]: nestedField
        ? { ...prevState[field], [nestedField]: value }
        : value,
    }));
  };
  ...
  return (
    <Container sx={{ mt: 3 }}>
      ...
      <Typography variant="h6" gutterBottom sx={{ color: "#303f9f" }}>
        {selectedType
          ? `Music of type: ${selectedType}`
          : "Select a type to see the music"}
      </Typography>
      {list.length > 0 ? (
            ...
            <TableBody>
              {list.map((item, index) => (
                <TableRow key={item.idMusic}>
                  ...
                  <TableCell>
                    <IconButton
                      onClick={(event) => handleClickMore(event, item)}
                    >
                      <MoreVert />
                    </IconButton>
                    <Menu
                      ...
                      open={
                        Boolean(anchorEl) &&
                        currentItem?.idMusic === item.idMusic
                      }
                      onClose={handleClose}
                    >
                      <MenuItem onClick={handleOpenEditDialog}>Edit</MenuItem>
                      <MenuItem onClick={() => handleDeleteItem(item.idMusic)}>
                        Delete
                      </MenuItem>
                    </Menu>
                  </TableCell>
